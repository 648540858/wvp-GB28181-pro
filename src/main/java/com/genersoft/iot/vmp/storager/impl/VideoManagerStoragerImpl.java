...
public class VideoManagerStoragerImpl implements IVideoManagerStorager {
    ...
    @Override
    public Page<DeviceChannel> queryChannelsByDeviceId(String deviceId, String query, Boolean hasSubChannel, Boolean online, int pageNo, int pageSize) {
        PageHelper.startPage(pageNo, pageSize);
        List<DeviceChannel> all = deviceChannelMapper.queryChannelsByDeviceId(deviceId, null, query, hasSubChannel, online);
        return new Page<>(new PageInfo<>(all));
    }
     ...
     */
    @Override
    public Page<Device> queryVideoDeviceList(int page, int count, Device device) {
        PageHelper.startPage(page, count);
        List<Device> all = deviceMapper.getDevices(device);
        PageInfo<Device> pageInfo = new PageInfo<>(all);
    ...
    @Override
    public Page<ParentPlatform> queryParentPlatformList(int page, int count) {
        PageHelper.startPage(page, count);
        List<ParentPlatform> all = platformMapper.getParentPlatformList();
        Page<ParentPlatform> newPage = new Page<>(new PageInfo<>(all));
        return newPage;
    }
     ...
     */
    @Override
    public Page<StreamProxyItem> queryStreamProxyList(Integer page, Integer count, String query, Boolean enable) {
        PageHelper.startPage(page, count);
        List<StreamProxyItem> all = streamProxyMapper.selectAll(query, enable);
        Page<StreamProxyItem> newPage = new Page<>(new PageInfo<>(all));
        return newPage;
    }
