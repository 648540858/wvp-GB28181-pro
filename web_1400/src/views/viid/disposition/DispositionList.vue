  ...
  <el-card class="box-card" shadow="always">
      <el-form ref="queryForm" @submit.native.prevent :model="queryParams" :inline="true">
          <el-form-item label="布控标题" prop="title">
              <el-input v-model="queryParams.title" placeholder="请输入布控标题" clearable size="small"
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                  <el-popover placement="left" trigger="click">
                      <!-- <el-button size="mini" type="text" icon="el-icon-edit-outline" @click="handleEdit(scope.row)">修改</el-button> -->
...
<script>
import { pageDisposition, delDispositions } from '@/api/datawork/viid/disposition'

export default {
  name: 'VIIDDispositionList',
  data() {
      return {
          tableHeight: null,
          // 展示切换
          ...
          // 表格头
          tableColumns: [
              { prop: 'dispositionId', label: '布控标识', show: true },
              { prop: 'title', label: '布控标题', show: true },
              { prop: 'dispositionCategory', label: '布控类别', show: true },
              { prop: 'targetFeature', label: '目标特征', show: false },
              { prop: 'targetImageUri', label: '目标图片', show: true },
              { prop: 'priorityLevel', label: '优先等级', show: true },
              { prop: 'applicantName', label: '应用名称', show: false },
              { prop: 'applicantInfo', label: '应用信息', show: false },
              { prop: 'applicantOrg', label: '应用组织机构', show: false },
              { prop: 'beginTime', label: '布控开始时间', show: false },
              { prop: 'endTime', label: '布控结束时间', show: false },
              { prop: 'dispositionStatus', label: '布控状态', show: true },
              { prop: 'dispositionRange', label: '布控范围', show: true },
              { prop: 'tollgateList', label: '布控卡口', show: false },
              { prop: 'dispositionArea', label: '布控行政区域', show: false },
              { prop: 'receiveAddr', label: '告警信息接收地址', show: false },
              { prop: 'receiveMobile', label: '告警信息接收手机号', show: false },
              { prop: 'reason', label: '布控理由', show: false }
          ],
          // 默认选择中表格头
          checkedTableColumns: [],
          tableSize: 'medium',
  ...
  methods: {
      ...
      getList() {
          this.loading = true
          pageDisposition(this.queryParams).then(response => {
              this.loading = false
              if (response.success) {
      ...
      handleBatchDelete() {
          ...
          this.$confirm('选中数据将被永久删除, 是否继续？', '提示', {
            ...
            delDispositions(this.ids).then(response => {
                  if (response.success) {
                      this.$message.success('删除成功')
                      this.getList()
                  }
