<template>
    <el-card class="box-card" shadow="always">
        ...
        <el-row type="flex" justify="space-between">
            <el-col :span="12">
                <el-button-group>
                    <el-button v-hasPerm="['viid:nonvehicle:add']" type="primary" icon="el-icon-plus" size="mini"
                        @click="handleAdd">新增</el-button>
                    <el-button v-hasPerm="['viid:nonvehicle:edit']" type="success" icon="el-icon-edit-outline" size="mini"
                        :disabled="single" @click="handleEdit">修改</el-button>
                    <el-button v-hasPerm="['viid:nonvehicle:remove']" type="danger" icon="el-icon-delete" size="mini"
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    <el-popover placement="left" trigger="click">
                        <el-button v-hasPerm="['viid:nonvehicle:edit']" size="mini" type="text" icon="el-icon-edit-outline"
                            @click="handleEdit(scope.row)">修改</el-button>
                        <el-button v-hasPerm="['viid:nonvehicle:remove']" size="mini" type="text" icon="el-icon-delete"
...
<script>
import { pageNonMotorVehicle, delNonMotorVehicles } from '@/api/datawork/nonmotorvehicle'

export default {
    name: 'VIIDNonMotorVehicleList',
    data() {
        return {
            tableHeight: null,
            // 展示切换
            ...
            // 表格头
            tableColumns: [
                { prop: 'nonMotorVehicleId', label: '非机动车标识', show: true },
    ...
    methods: {
        ...
        getList() {
            this.loading = true
            pageNonMotorVehicle(this.queryParams).then(response => {
                this.loading = false
                if (response.success) {
        ...
        handleDelete(row) {
            this.$confirm('选中数据将被永久删除, 是否继续？', '提示', {
                ...
                delNonMotorVehicles(row.id).then(response => {
                    if (response.success) {
        ...
        handleBatchDelete() {
            ...
            this.$confirm('选中数据将被永久删除, 是否继续？', '提示', {
                ...
                delNonMotorVehicles(this.ids).then(response => {
                    if (response.success) {
                        this.$message.success('删除成功')
                        this.getList()
                    }
